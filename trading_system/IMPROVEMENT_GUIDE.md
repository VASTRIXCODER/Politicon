...
| **5** | **Frontier feeds**: two specialised background engines + dashboard views, each with a clean **data-client → scanner split** (mirroring `data.py` → `scanner.py`). **Crypto**: `dexscreener.py` (client) + `crypto_scanner.py` (a ~70-token categorised universe + a 4-factor signal: momentum/pressure/turnover/liquidity), with **inline DexScreener charts** + category filters. **Polymarket**: `polymarket.py` (client) + `polymarket_scanner.py` (categorisation, classification, biggest-movers + closing-soon grouping, probability bars). App-native (public APIs direct; MCP servers in `.mcp.json` for the agent layer). Analysis only — no on-chain / Polygon orders | **Implemented** |

| **6** | **Variable position cap + Auto-Tune.** `MAX_OPEN_POSITIONS` can be computed live (`DYNAMIC_POSITIONS`) from account equity **and** a market-regime score (`risk.market_regime`: breadth + trend + conviction) — more positions in a broad uptrend, fewer when risk-off, bounded by how many fit at `MAX_POSITION_PCT`. The engine recomputes the regime each cycle and feeds equity+regime into `can_open_new`. A one-click **Auto-Tune** button reads the current regime and applies the best *risk-adjusted* combination (entries, ATR sizing, dynamic cap) — explicitly **not** a profit guarantee | **Implemented** |
